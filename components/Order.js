...
const Order = ({ order }) => {
  ...
  const [DeleteOrder] = useMutation(DELETE_ORDER, {
    update(cache) {
      ...
      cache.writeQuery({
        ...
        data: {
          getOrderSeller: getOrderSeller.filter(
  ...
  return (
    <div
      className={`${classString} border-t-4 mt-4 bg-gray-300 rounded p-6 md:grid md:grid-cols-2 md:gap-4`}
    >
      <div>
      ...
      <div>
        ...
        <button
          className='flex items-center mt-4 bg-red-800 px-5 py-2 inline-block text-white leading-tight uppercase text-xs font-bold'
